...
func renderMiniPiece(p *Piece) [4]string {
	grayCell := lipgloss.NewStyle().Foreground(ui.ColorSubtle).Render("··")
...
func (t *Tetris) renderBoardLines() []string {
	...
	subtle := lipgloss.NewStyle().Foreground(ui.ColorSubtle)
	ghostStyle := lipgloss.NewStyle().Foreground(ui.ColorDimText)
	...
	for y := 0; y < BoardHeight; y++ {
		...
		for x := 0; x < BoardWidth; x++ {
			...
			case ghostCells[pos]:
				row.WriteString(ghostStyle.Render("░░"))
			default:
				row.WriteString(subtle.Render("··"))
			}
		}
		row.WriteString("  " + border.Render("║"))
...
func (t *Tetris) renderSidebarLines() []string {
	...
	dim := lipgloss.NewStyle().Foreground(ui.ColorDimText)
	emph := lipgloss.NewStyle().Foreground(ui.ColorEmphasis)

	sideW := 14 // inner visible width (must match ╔══════════════╗ inner)

	emptyRow := strings.Repeat(" ", sideW)
	...
	// HOLD label: accent for the word, dim indicator when empty
	var holdLabelContent string
	if t.held != nil {
		holdLabelContent = accent.Render("    HOLD") + "      "
	} else {
		holdLabelContent = accent.Render("    HOLD") + dim.Render("  ----")
	}

	lines := []string{
		border.Render("╔══════════════╗"),
		fullRow(accent.Render("    NEXT") + "      "),
		fullRow(emptyRow),
		miniRow(nextGrid[0]),
		miniRow(nextGrid[1]),
		miniRow(nextGrid[2]),
		...
		fullRow(emptyRow),
		sepRow,
		// Stat rows: label in dim, value in emphasis — each exactly 14 visible chars
		fullRow(dim.Render(" SCORE:") + emph.Render(fmt.Sprintf("%-7d", t.score))),
		fullRow(dim.Render(" LEVEL: ") + emph.Render(fmt.Sprintf("%-6d", t.level))),
		fullRow(dim.Render(" LINES: ") + emph.Render(fmt.Sprintf("%-6d", t.lines))),
		fullRow(emptyRow),
		border.Render("╚══════════════╝"),
	}
	return lines
...
// styledControlRow renders a single controls row with keys like [←→] in accent
// color and descriptions in dim text. s must be exactly inner runes wide.
func styledControlRow(s string, keyStyle, descStyle lipgloss.Style) string {
	runes := []rune(s)
	...
	for i < len(runes) {
		if runes[i] == '[' {
			...
			if j < len(runes) {
				result.WriteString(keyStyle.Render(string(runes[i : j+1])))
				i = j + 1
				continue
			}
		}
		...
		result.WriteString(descStyle.Render(string(runes[i:j])))
		i = j
	}
...
func renderControlsLines() []string {
	border := lipgloss.NewStyle().Foreground(ui.ColorBorder)
	keyStyle := lipgloss.NewStyle().Foreground(ui.ColorAccent)
	descStyle := lipgloss.NewStyle().Foreground(ui.ColorDimText)

	inner := 42
	row := func(s string) string {
		runes := []rune(s)
		// Pad to exactly inner runes; truncate if somehow over
		for len(runes) < inner {
			runes = append(runes, ' ')
		}
		if len(runes) > inner {
		...
		return border.Render("║") + styledControlRow(string(runes), keyStyle, descStyle) + border.Render("║")
	}

	// Each row must be ≤ 42 runes. Counts verified by rune position:
	//   row1: 42 runes  (· represents Space/hard-drop)
	//   row2: 40 runes + 2 pad
	//   row3: 21 runes + 21 pad
	return []string{
		border.Render("╔══════════════════════════════════════════╗"),
		row(" [←→] Move  [↑] Rotate  [↓] Soft  [·] Hard"),
		row(" [C] Hold  [Z] Swap  [P] Pause  [Q] Quit"),
		border.Render("╚══════════════════════════════════════════╝"),
	}
}

// renderTitleLine returns the single-line title banner (44 visible chars to align
// with the controls footer box).
func renderTitleLine() string {
	borderStyle := lipgloss.NewStyle().Foreground(ui.ColorBorder)
	titleStyle := lipgloss.NewStyle().Foreground(ui.ColorAccent)
	// 15 ═ + space + "T E T R I S" (11) + space + 16 ═ = 44 chars
	return borderStyle.Render(strings.Repeat("═", 15)) + " " +
		titleStyle.Render("T E T R I S") + " " +
		borderStyle.Render(strings.Repeat("═", 16))
}
...
func (t *Tetris) Render() string {
	...
	sb.WriteString("\n")
	sb.WriteString("  ")
	sb.WriteString(renderTitleLine())
	sb.WriteString("\n")
	for i := range board {
		sb.WriteString("  ")
